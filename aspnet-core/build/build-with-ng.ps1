...
$webHostFolder = Join-Path $slnFolder "src/Rarkaine.Portal.Web.Host"
$webPublicFolder = Join-Path $slnFolder "src/Rarkaine.Portal.Web.Public"
...
docker rmi rarkaineportalwebhost -f
...
docker rmi rarkaineportalwebpublic -f
docker compose -f docker-compose.yml build

# Angular UI
Set-Location (Join-Path $outputFolder "ng")

docker rmi rarkaineportalwebangular -f
docker compose -f docker-compose.yml build
